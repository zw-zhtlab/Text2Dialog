...
- **长文本分块**：按 Token 上限智能分块，支持跨块重叠上下文，减轻切断语义带来的误判。
...
以下为基于 Text2Dialog 构建数据并微调后的 LLM 一个模型间对话片段：
...
LLM_PLATFORM=openai
```
> 前端“高级设置”亦支持临时覆盖（平台、API Key、BaseURL、模型名），仅对本次作业生效。
...
```
- `reply` 可为 `null` 或对象；若为对象，`target_index` 必须早于当前 `dialogue_index` 且在同一 chunk 内。
...
- `--reply-window`：reply 可回溯窗口（仅看本 chunk 内的更早发言）。
...
```
常用参数要点：
- `--pairs` / `--roles` + `--all-ordered-pairs`：显式或按角色集全排列生成有序对（自反对除外）。
- 过滤：`--min-confidence`（默认 0.8）、`--require-confidence`、文本长度上下限与 `--deny-pattern` 黑名单正则。
- **严格模式**（默认启用）：仅保留能确定为 `A→B` 的样本；`--no-strict` 可放宽到启发式回退。
...
- **校验规则**（节选）：
  ...
  - `role` 与 `dialogue`：非空。
  - `reply`：`null` 或对象；若为对象：`target_index` 早于当前且位于同 chunk；`confidence ∈ [0,1]`。
